...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
		...
		// ECRM fix: the bundled .mtx.php cache may bake a RELATIVE $ttffile path,
		// which fails when embedding/subsetting at Output time on any server whose
		// CWD is not the plugin root. Force the absolute path computed from fontpath.
		$ttffile = $ttffilename;
