...
class ECRM_Import {
	...
	public static function apply( array $pairs, bool $dry, int $provider_id = 0 ): array {
		...
		foreach ( $pairs as $pair ) {
			...
			// Ο ειδικός έλεγχος για ψεύτικο «Υπογράφηκε» που ζούσε εδώ από τις
			// 2026-08-24 αφαιρέθηκε στις 07/09: η κατάσταση `signed` δεν
			// υπάρχει πια, και το κενό που φύλαγε το κλείνει πλέον η
			// `PaperworkGate` μέσα στον ίδιο τον `ContractLifecycle` -- για
			// ΟΛΕΣ τις πόρτες μαζί, αυτήν εδώ συμπεριλαμβανομένη, και
			// απαιτώντας υπογραφή για κάθε φυλασσόμενη κατάσταση αντί για μία.
			if ( ! $dry ) {
				...
				// στον πελάτη, και το ecrm_contract_status_changed στο οποίο
				// κρέμονται οι ακροατές του κύκλου ζωής.
				//
