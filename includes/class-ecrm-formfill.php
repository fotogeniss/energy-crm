...
class ECRM_FormFill {
	...
	public static function values( array $c ): array {
		...
		return $mobile + [
			...
			// Η Συνδυαστική (family) δένει 2 γραμμές κινητής κάτω από το ίδιο
			// ΑΦΜ. Ώς τις 11/09/2026 η 2η γραμμή τυπωνόταν σε ξεχωριστή
			// σημείωση στη σελίδα 3 («ΓΙΑ ΤΟΝ ΣΥΝΕΡΓΑΤΗ»), μακριά από το
			// κουτί ΑΡΙΘΜΟΣ ΚΙΝΗΤΟΥ της σελίδας 1 που κοιτάζει ο πάροχος --
			// ζητήθηκε (264) να τυπώνονται και οι δύο αριθμοί μαζί ΕΚΕΙ.
			//
			// Δικό της κλειδί ('arithmoi_kinitou_synduastikou'), όχι ξανά
			// 'arithmos_kinitou': το κλειδί αυτό το μοιράζονται ΟΛΑ τα
			// πρότυπα κινητής (π.χ. orizon_mobile.json, η ίδια η σύμβαση) --
			// αν γραφόταν εδώ ο συνδυασμός, θα τυπωνόταν σιωπηλά και στο
			// κουτί της σύμβασης, όχι μόνο στο φύλλο της Συνδυαστικής. Το
			// νέο κλειδί υπάρχει ΜΟΝΟ στο orizon_family.json.
			//
			// Χωρίς 2η γραμμή, μένει μόνο ο πρώτος αριθμός -- ίδια
			// συμπεριφορά με πριν για το ίδιο κουτί. Ο έλεγχος mobile_offer
			// εδώ είναι δεύτερη γραμμή άμυνας, όχι εμπιστοσύνη στο frontend
			// (η JS καθαρίζει το mobile_msisdn_2 εκτός Συνδυαστικής).
			'arithmoi_kinitou_synduastikou' => (
				$xg( 'mobile_offer' ) === 'family' && $xg( 'mobile_msisdn_2' ) !== ''
					? $xg( 'mobile_msisdn' ) . ' / ' . $xg( 'mobile_msisdn_2' )
					: $xg( 'mobile_msisdn' )
			),
			...
			// ενότητας, όχι νέα δήλωση Ναι/Όχι. Επιβεβαιώνει ότι ο πελάτης
			// ενημερώθηκε για το δικαίωμα ανάκλησης συναίνεσης και καταγγελίας
			// στην Αρχή Προστασίας Δεδομένων (dpa.gr).
			'gdpr_rights_ack'          => ( $xg( 'gdpr_rights_ack' ) === 'yes' ? 'X' : '' ),
