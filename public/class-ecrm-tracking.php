...
class ECRM_Tracking {
	...
	public static function rest_sign( WP_REST_Request $req ): WP_REST_Response {
		...
		if ( ! in_array( $row['status'], self::signable_statuses(), true ) ) {
			return new WP_REST_Response( [ 'ok' => true, 'already' => true, 'message' => 'Η αίτηση δεν εκκρεμεί για υπογραφή ή έχει ήδη υπογραφεί.' ], 200 );
		}

		// 04/09 (3β-Β, δεύτερο εύρημα): το `! empty( $row['signed_at'] )` που
		// ζούσε εδώ μέχρι σήμερα έλεγχε τη ΛΑΘΟΣ ερώτηση -- "υπέγραψε ΠΟΤΕ
		// αυτή η σύμβαση πλήρως" αντί για "χρειάζεται ΤΩΡΑ υπογραφή από ΑΥΤΟΝ
		// τον ρόλο". Μια COMBO αίτηση που υπέγραψε πλήρως όσο ήταν ακόμα
		// μονο-προσωπική (ένας απαιτούμενος ρόλος) και μετά επεξεργάστηκε να
		// θέλει και δεύτερο, κρατά το παλιό `signed_at` -- το
		// `SignLinkController::create()` το καθαρίζει ΜΟΝΟ όταν ξαναστέλνει
		// σε ρόλο που ΕΙΧΕ ήδη υπογράψει (`confirm_resend`), όχι όταν στέλνει
		// σε νέο, ΑΚΟΜΑ ανυπόγραφο ρόλο. Αποτέλεσμα: ο πελάτης υπέγραφε εδώ,
		// έπαιρνε "ok" 200, και τίποτα δεν αποθηκευόταν ποτέ -- ο έλεγχος
		// έβγαινε πριν καν φτάσει στο σημείο που γράφει την υπογραφή.
		//
		// Ο ανά-ρόλο έλεγχος από κάτω (`SignatureState::forContract()`,
		// ίδιο σημείο αλήθειας με τον SignLinkController) είναι ήδη ακριβώς
		// αυτό που χρειάζεται: αν ο ρόλος έχει ΗΔΗ υπογράψει, "already"·
		// αλλιώς προχωράει. Δεν χρειάζεται δεύτερος, χοντρικός έλεγχος πάνω
		// από αυτόν -- απλώς έκρυβε τη σωστή απάντηση.
		$state = self::signature_state( $id, $row );
