...
>
> **Έκλεισε 2026-08-13.** Το οβάλ «Πάγια εντολή εξόφλησης μέσω τραπεζικού
> λογαριασμού» (`pliromi_pagia_entoli` ← `extra_json.payment_method`) έχει πλέον
> συντεταγμένη και στα **δέκα** έντυπα που το φέρουν στο χαρτί.
>
> Επί μήνες την είχαν μόνο τέσσερα — `nrg_he`, `nrg_he_biz`, `nrg_fa`,
> `protergia_fa`. Πελάτης που επέλεγε πάγια εντολή σε **ρεύμα Protergia** έπαιρνε
> έντυπο με το κουτάκι άδειο, ενώ στο αέριο του ίδιου παρόχου τσεκαριζόταν
> κανονικά. Κανένα σφάλμα: fill key χωρίς συντεταγμένη απλώς δεν σχεδιάζεται.
>
> Η προηγούμενη διατύπωση εδώ έλεγε «δεν έχει συντεταγμένη σε **κανένα** έντυπο
> Protergia», που ήταν αναληθές και ήταν αυτό που το έκρυβε: «λείπει από παντού»
> διαβάζεται εκκρεμότητα, «λείπει μόνο στο ρεύμα ενώ δίπλα δουλεύει» διαβάζεται
> bug. Το `StandingOrderCoordinateTest` απαριθμεί πλέον τα δέκα ονομαστικά.
