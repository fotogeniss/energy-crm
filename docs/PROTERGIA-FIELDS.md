...
## Ποιο έντυπο τυπώνεται

Από 2026-08-09 η Protergia δίνει **τέσσερα** οικιακά έντυπα, ένα ανά τιμολόγιο,
στη θέση του ενιαίου `protergia_he` (Picasso 2.0). Ο πίνακας τιμολόγησης στη
σελ. 2 είναι τυπωμένος από τον πάροχο — το CRM δεν συμπληρώνει τίποτα εκεί.

| Πρόγραμμα (`programs.code`) | Έντυπο | Τιμολόγηση |
|---|---|---|
| `protergia_oik_sure12` | Value Sure 12 Μήνες 3.0 | Σταθερή, 9,90 €/μήνα, 269 €/MWh |
| `protergia_oik_sure18` | Value Sure 18 Μήνες 3.0 | Σταθερή, 11,90 €/μήνα, 259 €/MWh |
| `protergia_oik_lite2` | Value Lite 2.0 | Κυμαινόμενη, 0,00 €/μήνα |
| `protergia_oik_bright` | Value Bright | Κυμαινόμενη, 5,00 €/μήνα |

Το `code` του προγράμματος **είναι** το κλειδί του εντύπου — μία ονομασία, όχι
δύο που μπορούν να ξεσυγχρονιστούν (`ProtergiaHomePlans`). Σύμβαση σε πρόγραμμα
εκτός των τεσσάρων (παλιά, ή φτιαγμένο από τον χρήστη) τυπώνει το `protergia_he`:
ό,τι πουλήθηκε, τυπώνεται.

> Το `ECRM_FormFill::template_key()` διαβάζει `program_code`, **όχι**
> `program_name`. Το όνομα είναι ελεύθερο κείμενο που αλλάζει από το admin· αν
> κρινόταν από αυτό, μια μετονομασία θα άλλαζε σιωπηλά το έντυπο που υπογράφει
> ο πελάτης.

Και τα τέσσερα είναι εξασέλιδα: 3 σελίδες αίτησης + 3 σελίδες Γενικών/Ειδικών
Όρων. Οι Όροι ήταν παλιότερα χωριστό συνοδευτικό έντυπο εκτός CRM· τώρα
τυπώνονται μαζί, οπότε δεν χρειάζεται ξεχωριστό συνημμένο.
...
> Το `isxis_paroxis` και το `poso_eggiisis` **δεν** είχαν ποτέ συντεταγμένη στο
> `protergia_he`, παρότι ο πίνακας εδώ τα ανέφερε: το κελί έμενε κενό και
> κανένα σφάλμα δεν εμφανιζόταν. Στα τέσσερα νέα οικιακά έντυπα τυπώνονται
> κανονικά (σελ. 1 και σελ. 2 αντίστοιχα).
>
> **Εκκρεμεί, και η προηγούμενη διατύπωση εδώ ήταν λάθος.** Έλεγε ότι το οβάλ
> «Πάγια εντολή εξόφλησης μέσω τραπεζικού λογαριασμού»
> (`pliromi_pagia_entoli` ← `extra_json.payment_method`) δεν έχει συντεταγμένη
> σε **κανένα** έντυπο Protergia. Έχει: στο `protergia_fa` (αέριο), σελ. 1,
> `x 141.2 / y 94.5`, `"check": true`.
>
> Λείπει μόνο από τα **Protergia ρεύματος** — `protergia_he`,
> `protergia_he_biz` και τα τέσσερα `protergia_oik_*`. Δηλαδή πελάτης που
> επιλέγει πάγια εντολή σε ρεύμα Protergia παίρνει έντυπο με το κουτάκι άδειο,
> ενώ στο αέριο του **ίδιου παρόχου** τσεκάρεται κανονικά. Αυτό δεν είναι
> εκκρεμότητα, είναι bug — και το «σε κανένα» ήταν ακριβώς αυτό που το έκρυβε.
>
> Είναι εντολή χρέωσης τραπεζικού λογαριασμού, οπότε πριν μπει συντεταγμένη
> χρειάζεται επιβεβαίωση ότι το οβάλ υπάρχει πράγματι πάνω σε αυτά τα έξι
> έντυπα. Παρακολουθείται στο `HANDOVER.md` §6α.

## Αίτηση

| Κλειδί | Τι τυπώνει | Πηγή |
|---|---|---|
| `arithmos_aitisis` | **Δεν τυπώνεται πια** (2026-08-09) — το «Κωδικός Αίτησης» στο έντυπο του παρόχου είναι δικό του πεδίο· ο εσωτερικός κωδικός της CRM (`contracts.code`, π.χ. PROTERGIA-0001) δεν έχει νόημα εκεί. Ο κωδικός παραμένει κανονικά στη βάση/CRM. | — |
...
| `synainesi_omilou_nai` / `_oxi` | όρος **Ζ** (προωθητικοί σκοποί) = `extra_json.group_data_consent` |
| `synainesi_erevnas_nai` / `_oxi` | όρος **Η** (τηλεφωνικές έρευνες) = `extra_json.survey_consent` |

> Οι δύο συναινέσεις μπαίνουν μόνο στα τέσσερα νέα οικιακά έντυπα. Αναπάντητη
> ερώτηση δεν τυπώνει **κανένα** από τα δύο κουτιά: κενό είναι σωστότερο από
> μια συναίνεση που δεν ζητήθηκε ποτέ.
