...
(285) Το `poso_eggiisis` ήταν στο x = 113, πάνω στο κουτάκι «Καταβολή εγγύησης
με τον 1ο λογαριασμό» -- μεταφέρθηκε δίπλα στη δική του ετικέτα.
...
| 1 | 29.5 | 173.7 | `poso_eggiisis` | κείμενο | Ποσό εγγύησης (€) |

## protergia_fa_sure / protergia_fa_single — Φυσικό αέριο, ένα έντυπο ανά τιμολόγιο (285)

Αρχεία: `assets/forms/protergia/protergia_fa_sure.json` (5 σελ.),
`assets/forms/protergia/protergia_fa_single.json` (4 σελ.).

**Sure** (ΕΟ.1250-2_94): σελ. 1-2 είναι pixel προς pixel το `protergia_fa` (διαφορά
0,00% / 0,02% στα 150 dpi), σελ. 3-5 οι ΓΟΣ. Ο χάρτης είναι **byte προς byte ο ίδιος**
με το `protergia_fa.json` -- το τεστ `ProtergiaGasPlansTest` το απαιτεί. Το κουτάκι
«Οικιακό Αυτόνομο» είναι ήδη τυπωμένο τσεκαρισμένο στο έντυπο.

**Single Value** (ΕΟ.1250-2_87): ίδιο πλέγμα σελ. 1 (όλες οι ετικέτες στα ίδια mm με
το Sure, ελεγμένο με PyMuPDF), ίδια θέση υπογραφής στη σελ. 2. Επιπλέον δύο κουτιά
τιμολογίου, που το Sure δεν έχει:

| σελ | x | y | κλειδί | τύπος | ετικέτα |
|----:|----:|----:|---|---|---|
| 1 | 33.6 | 161.7 | `fa_aftonomo` | ☑ X | Φυσικό Αέριο Οικιακό Αυτόνομο |
| 1 | 84.2 | 161.7 | `fa_koinoxristo` | ☑ X | Φυσικό Αέριο Οικιακό Κοινόχρηστο |

(οβάλ 30.9-38.4 / 81.4-88.9 × 162.5-165.9 mm· x = κέντρο − 1.0, y = πάνω − 0.8.)
Γεμίζουν από την `category` της σύμβασης: `communal` → Κοινόχρηστο, οτιδήποτε άλλο
→ Αυτόνομο.
