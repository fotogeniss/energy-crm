 ...
 * τους, όχι με σταθερή ώρα) — θα περνούσε ΚΑΙ αν οι δύο ζώνες συνέπιπταν
 * τυχαία, οπότε από μόνος του δεν αποδεικνύει τίποτα για τη μέθοδο.
 * **Μετρήθηκε 23/08: εδώ δεν συμπίπτουν.** Η MySQL (`@@session.time_zone =
 * SYSTEM`, δηλαδή ζώνη Αθήνας) τρέχει τρεις ώρες μπροστά από το WordPress
 * της σουίτας (`timezone_string` κενό, `gmt_offset` 0 → UTC). Ο πρώτος
 * έλεγχος περνάει όχι επειδή οι ζώνες συμπίπτουν, αλλά επειδή και οι δύο
 * στήλες γράφονται πλέον αποκλειστικά από τη MySQL — καμία δεν διαβάζει
 * `current_time('mysql')`. Δες docs/CHANGELOG.md (96).
 *
