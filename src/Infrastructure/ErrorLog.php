 ...
 * περιεχόμενο αιτήματος, POST/GET, cookies, headers. Ενας καταγραφέας που τα
...
final class ErrorLog
{
    ...
    /** Οσα χωράνε σε μία οθόνη χωρίς να φουσκώσει το option. */
    private const KEEP = 50;

    /** Κόβει μήνυμα που ξέφυγε· κανένα δικό μας δεν πλησιάζει. */
    private const MAX_MESSAGE = 500;

    /** Ο,τι βλέπει ο χρήστης όταν φταίμε εμείς. Το %s είναι ο κωδικός. */
    ...
    private function record(string $kind, string $message, string $file, int $line): string
    {
        ...
        // κωδικούς. Ενα σφάλμα που επαναλαμβάνεται χίλιες φορές δεν φτιάχνει
        ...
        // Ιδιο σφάλμα στη σειρά μετριέται, δεν επαναλαμβάνεται: ένα cron που
