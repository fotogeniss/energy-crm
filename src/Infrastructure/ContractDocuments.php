...
final class ContractDocuments
{
     ...
     *
     * Δημόσια (04/09/2026, ίδια μέρα, δεύτερο εύρημα) γιατί το
     * `ContractDocumentsController::providerForm()` -- το χειροκίνητο κουμπί
     * «PDF έντυπο» -- χρειάζεται ΑΚΡΙΒΩΣ τον ίδιο χάρτη με το `store()`
     * παρακάτω, και μια δεύτερη γραφή του ίδιου βρόχου θα μπορούσε να
     * ξαναδιαφωνήσει με αυτή. Πριν τη διόρθωση το κουμπί περνούσε μόνο το
     * ιστορικό `$sig_path` (καμία υπογραφή ρόλου), οπότε οι δύο γραμμές
     * υπογραφής του COMBO έμεναν ΠΑΝΤΑ κενές σε ό,τι κατέβαζε ο πωλητής --
     * ενώ το αυτόματα αποθηκευμένο/σταλμένο αντίγραφο (μέσω αυτού εδώ του
     * store()) τις έδειχνε σωστά. Δύο διαδρομές, μία εικόνα δύο φορές
     * διαφορετική -- ό,τι ακριβώς το SignatureRoles υπάρχει για να αποτρέψει.
     *
     * @return array<string, string>
     */
    public function signaturesByRole(int $contractId): array
    {
        $map = [];
