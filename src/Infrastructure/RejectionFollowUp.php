 ...
 * `CancelledByUs` πάνω στη σύμβαση, και αναλαμβάνει από εκεί και πέρα.
 *
 * ## AUDIT 07/09 (μετά το commit 254): ΤΙ ΚΕΙΜΕΝΟ γράφει, και γιατί άλλαξε
 *
 * Το status `Rejected` που άκουγε αρχικά αυτή η κλάση ΕΦΥΓΕ στο (254) --
 * συγχωνεύτηκε στο `cancelled_by_us` μαζί με το γενικό `cancelled`
 * (`MigrateStatusVocabulary::MAP`). Το νέο `cancelled_by_us` καλύπτει και
 * τους **τρεις** λόγους ακύρωσης «από εμάς» του `docs/STATUS-MODEL.md` §7
 * (Δέσμευση συμβολαίου, Πολιτική αποδοχής, Δεν ολοκληρώθηκαν δικαιολογητικά)
 * -- όχι μόνο την απόρριψη παρόχου. Χωρίς στήλη `cancellation_reason` (έρχεται
 * στο commit 256) δεν υπάρχει τρόπος να ξεχωρίσει ΠΟΙΟΣ από τους τρεις λόγους
 * ισχύει σε κάθε συγκεκριμένη ακύρωση.
 *
 * Το κείμενο της εργασίας ΕΠΡΕΠΕ να άλλαζε μαζί με το σκοπό: μέχρι τις 07/09
 * έγραφε ρητά «Απορρίφθηκε από πάροχο -- ο πάροχος απέρριψε την αίτηση»,
 * πράγμα που είναι απλά ψέμα για μια ακύρωση λόγω δέσμευσης συμβολαίου ή
 * ημιτελών δικαιολογητικών. Δεν το έπιασε κανένα test -- δεν υπάρχει καθόλου
 * test αρχείο για αυτή την κλάση. Το κείμενο παρακάτω είναι πλέον γενικό,
 * αληθές για και τους τρεις λόγους· θα ξαναγίνει συγκεκριμένο (π.χ. μόνο για
 * «Πολιτική αποδοχής», όπως προέβλεπε αρχικά το CHANGELOG (254)) μόλις
 * υπάρξει η στήλη του λόγου να το ρωτήσει.
 *
 ...
 * σύμβαση σε CancelledByUs.
 *
 * ## Γιατί εργασία και όχι μόνο καμπανάκι
 *
 * Το `ContractNotices` ήδη χτυπά καμπανάκι για το CancelledByUs (είναι στο
 * δικό του `ANNOUNCED`) — αυτό ενημερώνει. Η εργασία εδώ είναι διαφορετική:
 * δεν λέει απλώς «κάτι έγινε», λέει «κάτι πρέπει να γίνει», και μένει στη
 * λίστα «Ανοιχτές» μέχρι κάποιος να την κλείσει ρητά — ο συνεργάτης δεν
 * μπορεί να τη χάσει σαν καμπανάκι σε λίστα από δέκα.
 *
 * ## Γιατί προθεσμία «σήμερα»
 *
 * Ρητή απόφαση ιδιοκτήτη (31/08): μια ακύρωση από εμάς είναι επιχειρηματικό
