...
final class SignatureRoles
{
    ...
    /**
     * Ποιος ρόλος ταυτίζεται με τον ΚΥΡΙΟ πελάτη της σύμβασης -- δηλαδή ποιανού
     * τα `contracts.email`/`mobile` μπορούν να χρησιμοποιηθούν απευθείας, χωρίς
     * τις στήλες `combo_*_mobile`/`combo_*_email` για "τον άλλο".
     *
     * Ώς το Στάδιο 4 (05/09/2026) η απάντηση ήταν πάντα MOBILE, κρυφά --
     * `SignLinkController`/`ContractsReadController` το είχαν καρφωμένο, γιατί
     * ο κύριος πελάτης ΗΤΑΝ πάντα ο πελάτης κινητής (η αίτηση ξεκινούσε πάντα
     * από Orizon). Το COMBO από αίτηση Volton αντιστρέφει αυτό: ο κύριος
     * πελάτης είναι ο πελάτης ΕΝΕΡΓΕΙΑΣ, και ο "άλλος" -- αν υπάρχει -- είναι
     * ο πελάτης κινητής. Οι δύο θέσεις υπογραφής στο χαρτί (mobile/energy) δεν
     * αλλάζουν καθόλου· αλλάζει μόνο ποια από τις δύο ταυτίζεται με τη γραμμή
     * της ίδιας της σύμβασης.
     *
     * **Ο κανονας δεν ειναι «το energy_type αποφασιζει».** Οταν απαιτειται ΜΙΑ
     * μονο υπογραφη (η συντριπτικη πλειοψηφια των αιτησεων: καθε απλη αιτηση
     * ρευματος, αεριου ή κινητης, και καθε COMBO ενος προσωπου) ο μοναδικος
     * απαιτουμενος ρολος ΕΙΝΑΙ ο κυριος πελατης -- εξ ορισμου, γιατι δεν
     * υπαρχει δευτερο προσωπο να ειναι. Το `requiredFor()` επιστρεφει εκει
     * παντα MOBILE, ιστορικα: το `doc_kind` 'signature' ειναι η γενικη θεση
     * υπογραφης καθε εντυπου, οχι «του πελατη κινητης» -- και δεν
     * μετονομαζεται, γιατι θα εκανε αναγνωστες ολα τα ηδη υπογεγραμμενα
     * αρχεια.
     *
     * Μονο οταν απαιτουνται ΔΥΟ υπογραφες υπαρχει πραγματικη ερωτηση «ποιος
     * απο τους δυο», και εκει την κρινει η ΠΡΟΕΛΕΥΣΗ της αιτησης.
     *
     * Χωρις αυτο το πρωτο σκελος, μια απλη αιτηση Volton (ενας υπογραφων,
     * ρολος MOBILE) θα θεωρουνταν «δευτερευων»: το SMS θα απορριπτοταν και το
     * email θα ζητουνταν απο τη στηλη `combo_mobile_email` -- κενη. Δηλαδη ο
     * πιο συνηθισμενος συνδεσμος υπογραφης του προϊοντος δεν θα εφευγε ποτε.
     *
     * @param string       $energyType `contracts.energy_type` -- 'mobile', 'power' ή 'gas'.
     * @param list<string> $required   Τι επεστρεψε το `requiredFor()` γι' αυτη τη συμβαση.
     */
    public static function primaryRoleFor(string $energyType, array $required): string
    {
        if (count($required) < 2) {
            return $required[0] ?? self::MOBILE;
        }

        return $energyType === 'mobile' ? self::MOBILE : self::ENERGY;
    }
