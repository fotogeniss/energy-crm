...
final class ProviderStatusMap
{
     ...
     * (28/08/2026) και ξαναγράφτηκαν στο νέο λεξιλόγιο (07/09/2026). Η σειρά
     * μετράει: το «ΠΡΟΣ ΥΠΟΓΡΑΦΗ» ταιριάζει με το `υπογρ` και με τίποτε άλλο,
     * αλλά μια μελλοντική προσθήκη πιο πάνω θα μπορούσε να το κλέψει.
     *
     * ## Δύο μοτίβα έφυγαν, και δεν αντικαταστάθηκαν
     *
     * Το `εκκρεμ|pending` και το `επιλ|resolv` έδειχναν σε καταστάσεις που
     * έγιναν **εμπόδια** -- ζουν πλέον σε δικό τους πίνακα, όχι στη στήλη
     * `status`. Ένας χάρτης που στοχεύει και τα δύο είδη είναι ξεχωριστή
     * δουλειά (`docs/STATUS-MODEL.md` §9.1)· ως τότε το κείμενο του παρόχου
     * απλώς δεν μαντεύεται, που είναι καλύτερο από το να μαντεύεται λάθος.
     *
     * Το `ακυρ|cancel` και το `απορρ|reject` δείχνουν και τα δύο στην ακύρωση
     * **από εμάς**: το Excel του παρόχου λέει ότι η αίτηση δεν προχώρησε, δεν
     * λέει ποιος το αποφάσισε. Το back office διορθώνει σε «από πελάτη» όταν
     * ισχύει -- και οι δύο είναι εξίσου μη πληρωτέες, οπότε η εικασία δεν
     * αγγίζει χρήματα.
     *
    ...
    private const GUESSES = [
        ...
        '/(ακυρ|cancel)/u'        => 'cancelled_by_us',
        '/(απορρ|reject)/u'       => 'cancelled_by_us',
        '/(δρομολ|rout|οριστ)/u'  => 'finalisation',
        '/(υπογρ|sign)/u'         => 'awaiting_signature',
        '/(sim|κάρτα|καρτα)/u'    => 'awaiting_sim',
        '/(επεξεργ|process|καταχ)/u' => 'registration',
        '/(τερματ|terminat|διακοπ)/u' => 'terminated',
        '/(νέα|νεα|new|presale)/u' => 'presale',
    ];
